...
median_day
```
So the mean value of steps every day is 10766 and the median value of steps per day is 10765.
...
Missing data needs to be imputed. A simple imputation was applied here by replacing NAs with average steps in its relavent interval across all days. For eamples, for interval 5 of 2012-10-02, the average step for "interval 5" of all days was used to replace the NA.
...
Then the new histogram on imputed data is created and mean and median of imputed data is calculated.
```{r new histogram}
...
```
So there're difference in mean and median values between raw data and imputed data.A histogram is shown below.
```{r difference}
